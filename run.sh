...
compare_teacher_selectors() {
    ...
    FIXED_PARAMS="--gpu_id $GPU_ID --rounds 50 --distill_data_source generated --sample_filter none --distill_mode kd"
...
compare_sample_filters() {
    ...
    FIXED_PARAMS="--gpu_id $GPU_ID --distill_data_source original --rounds 100 --teacher_selector random --distill_mode kd"
    
    # 基线：不进行任何样本筛选
    run_experiment "log_filter_none.txt" $FIXED_PARAMS --sample_filter none --distill_batch_size 32
...
grid_search_dkd() {
    ...
    FIXED_PARAMS="--gpu_id $GPU_ID --distill_data_source original --rounds 100 --teacher_selector all --sample_filter none --distill_mode dkd"

    # 使用嵌套循环遍历所有 alpha 和 beta 的组合
    for alpha in "${ALPHA_VALUES[@]}"; do
    beta=$(echo "1 - $alpha" | bc)
        # 为每个组合创建一个唯一的日志文件名
...
ResNet_test_generate() {
    echo "********* 开始运行 [ResNet generate_hard_scarcity]  *********"
    
    FIXED_PARAMS="--gpu_id $GPU_ID --distill_data_source original_scarcity --rounds 500 --distill_mode kd --sample_filter none"

    run_experiment "log_expert_top2.txt" $FIXED_PARAMS --teacher_selector expert_top2

    run_experiment "log_expert_top3.txt" $FIXED_PARAMS --teacher_selector expert_top3

    run_experiment "log_expert_all.txt" $FIXED_PARAMS --teacher_selector expert_all
